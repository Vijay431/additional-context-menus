...
- **Security audit remediation**: Added pnpm overrides for vulnerable dev-only transitive dependencies (`shell-quote`, `tmp`, `form-data`, `undici`, `linkify-it`, `markdown-it`, `js-yaml`, `esbuild`, `vite`) and regenerated `pnpm-lock.yaml` so `pnpm audit` reports zero vulnerabilities.
