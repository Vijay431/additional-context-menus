...
- **Security audit remediation**: Added a pnpm override pinning `nanoid` to `3.3.18` — the transitive copy pulled in by `postcss` (via `vite`/`vitest`) resolved to `nanoid@3.3.16`, which is vulnerable to indefinite looping in custom generators when `size` is zero ([GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8)). Pinned rather than open-ranged so the resolution stays within `postcss`'s `^3.3.6` peer requirement instead of jumping to `nanoid@6`. `pnpm audit --audit-level=low` now reports zero vulnerabilities.
