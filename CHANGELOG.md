...
### Fixed
- **Security audit remediation**: Added a pnpm override pinning `nanoid` to `>=3.3.18` (`<3.3.18` → `>=3.3.18`) — the prior transitive resolution (`nanoid@3.3.16`, pulled in by `vite>postcss>nanoid`) was vulnerable to GHSA-2v37-7h3g-55p8 (custom generators can loop indefinitely when size is zero). `pnpm audit --audit-level=low` now reports zero vulnerabilities.
