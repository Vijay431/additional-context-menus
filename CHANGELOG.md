...
### Fixed
- **Security audit remediation**: Added pnpm override for `nanoid` (`>=3.3.18 <4.0.0`) — `postcss`'s transitive `nanoid@3.3.16` was vulnerable to indefinite looping when a custom generator's size is zero ([GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8)). The range is capped below `4.0.0` to stay within `postcss`'s expected `^3.3.16` dependency and avoid pulling in the ESM-only `nanoid` v6 line. `pnpm audit --audit-level=low` now reports zero vulnerabilities.
