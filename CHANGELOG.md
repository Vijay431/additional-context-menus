...
- **Security audit remediation**: Added a pnpm override pinning `nanoid` to `^3.3.18` (was resolving to `3.3.16` via `postcss` → `vite` → `vitest`) — fixes [GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8) (high severity: custom generators can loop indefinitely when size is zero). `pnpm audit --audit-level=low` now reports zero vulnerabilities.
