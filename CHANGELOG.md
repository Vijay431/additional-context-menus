...
- **Security audit remediation**: Added a `nanoid` override (`>=3.3.17 <4.0.0`) for the transitive `postcss > nanoid` dependency — the resolved `nanoid@3.3.16` was vulnerable to indefinite looping in custom generators (GHSA-2v37-7h3g-55p8). The range is capped below `4.0.0` because `postcss@8.5.25` declares `nanoid: ^3.3.16` and an unbounded floor override resolved `nanoid` to the incompatible major `6.0.1`; `>=3.3.17 <4.0.0` stays within postcss's declared range while excluding the vulnerable window. `pnpm audit --audit-level=low` now reports zero vulnerabilities.
