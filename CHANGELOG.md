...
### Fixed
- **Security audit remediation**: Added a pnpm override for `nanoid` (`>=3.3.18 <4`) — the transitive `postcss` → `nanoid@3.3.16` resolution was vulnerable to an infinite loop in custom generators with `size: 0` ([GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8)). The upper bound keeps the resolution within `postcss`'s expected `nanoid` 3.x major (an unbounded `>=3.3.18` override resolves to `nanoid@6.0.1`, which is ESM-only and breaks `postcss`'s CommonJS `require`). `pnpm audit --audit-level=low` now reports zero vulnerabilities.
