...
### Fixed
- **Security audit remediation**: Added a pnpm override pinning `nanoid` to `>=3.3.18 <4.0.0` — the prior transitive resolution (`nanoid@3.3.16`, pulled in via `postcss` → `vite` → `vitest`) allowed custom generators to loop indefinitely when size is zero ([GHSA-2v37-7h3g-55p8](https://github.com/advisories/GHSA-2v37-7h3g-55p8)). `pnpm audit --audit-level=low` now reports zero vulnerabilities.
